...
export async function POST(req) {
  try {
    ...
    // 1. EMAIL VERIFICATION & INITIAL PING
    if (type === 'verify_email') {
      
      // A. The user needs to verify their email. Send them the link.
          ...
          try {
            await fetch('https://onesignal.com/api/v1/notifications', {
              ...
              body: JSON.stringify({
                ...
                contents: { en: `${firstName} wants a ${booking.service_name} at ${booking.booking_time}` },
                url: `${baseUrl}/dashboard`,
                priority: 10 // High Priority to wake up device
              })
            });
            console.log("Verified booking push notification sent to shop:", booking.shop_id);
          } catch (pushError) {
