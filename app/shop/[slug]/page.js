"use client";
import React, { useEffect, useState, use } from "react"; // Added 'use' for Next.js 15
import { 
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 pb-20">
      ...
      <div className="max-w-3xl mx-auto px-6 -mt-20 relative z-30">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 p-8 border border-slate-50 mb-8 text-center">
          ...
          <h1 className="text-4xl font-black tracking-tight mb-2">{shop.name}</h1>
          <p className="flex items-center justify-center gap-2 text-slate-500 font-bold text-sm">
            <MapPin size={16} className="text-red-500" /> {shop.address}, {shop.postcode}
          </p>

          <div className="flex justify-center gap-3 mt-6">
            <a href={`https://wa.me/${shop.whatsapp_number}?text=${whatsappMsg}`} target="_blank" className="bg-[#25D366] text-white p-3 rounded-full hover:scale-105 transition-transform shadow-lg shadow-green-100"><MessageSquare size={20} /></a>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
          ...
          <a href={`https://wa.me/${shop.whatsapp_number}?text=${whatsappMsg}`} target="_blank" className="flex-1 bg-[#25D366] text-white font-black py-6 rounded-[2rem] shadow-lg active:scale-95 transition-all flex items-center justify-center gap-3 text-xl">
            <MessageSquare /> WhatsApp
          </a>
        </div>

        {/* 4. SERVICE MENU (The List You Created in Dash) */}
        <div className="mb-10">
          <h3 className="font-black text-xl mb-6 flex items-center gap-2">Select Service</h3>
          <div className="grid gap-3">
            {(shop.service_menu || []).map((service, idx) => (
              <button key={idx} onClick={() => handleBookService(service)} disabled={!shop.is_open} className="w-full group bg-white border-2 border-slate-50 rounded-3xl p-5 hover:border-blue-600 transition-all active:scale-95 flex items-center justify-between text-left disabled:opacity-50">
                 <div>
                   <h4 className="font-black text-lg group-hover:text-blue-600">{service.name}</h4>
                   <p className="text-slate-400 text-xs font-bold mt-1"><Clock size={12} className="inline mr-1"/> {service.duration} mins</p>
                 </div>
                 <span className="font-bold text-sm">£{service.price}</span>
              </button>
            ))}
            {(!shop.service_menu || shop.service_menu.length === 0) && (
                <div className="text-center text-slate-400 font-bold py-10 bg-slate-50 rounded-3xl">No services listed yet.</div>
            )}
          </div>
        </div>
      </div>
      ...
      {isBookingOpen && (
        <BookingModal 
          ...
          services={shop.service_menu || []} // <--- PASSES THE REAL MENU
