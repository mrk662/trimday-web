...
export default function JoinPlatform() {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      // 🔥 UPDATED: Added emailRedirectTo and role metadata
      const { data: authData, error: authError } = await supabase.auth.signUp({
        email: email.trim().toLowerCase(),
        password: password,
        options: {
          emailRedirectTo: `${window.location.origin}/dashboard`, // Barber goes to dashboard after verify
          data: {
            shop_name: shopName.trim(),
            user_role: 'barber' // Tags the user as a barber
          }
        }
      });
      ...
      // Link the shop record to the Auth ID
      ...
      if (shopError) {
        if (shopError.code === '23505') {
          alert("This email or number is already registered. Please log in.");
          setLoading(false);
          return;
        }
        throw shopError;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center py-12 px-4 font-sans text-slate-900 text-left">
      ...
      <div className="max-w-xl w-full text-center mb-12">
        ...
        <h1 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter mb-4 leading-none text-slate-900">
      ...
      <div className="max-w-xl w-full bg-white rounded-[3rem] shadow-xl p-8 md:p-12 border border-slate-100">
          ...
          <div className="space-y-6 text-slate-900">
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep(1)} className="flex-1 bg-slate-100 text-slate-600 font-bold py-5 rounded-2xl uppercase text-xs italic">Back</button>
        ...
        {step === 3 && (
          <div className="space-y-8 text-center text-slate-900">
            ...
            <button onClick={() => setStep(2)} className="text-slate-400 font-bold hover:text-slate-600 transition-colors uppercase text-[10px] tracking-widest italic">Edit Details</button>
          </div>
        )}
      </div>
    </div>
